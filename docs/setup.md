...
The small live JSON-mode request passed on the fallback model and was recorded without the key or source payload. Earlier Qwen attempts failed with 429/rate-limit behavior and reasoning-token exhaustion, so the app configuration now uses the fallback model. Cloud Run verified runtime secret injection and protected app/API behavior; hosted reading and mathematics journeys passed on runtime `d1931fa`.
...
For class access, use `--no-invoker-iam-check` only after the application enforces HTTP Basic authentication on the UI and all AI routes. Missing or incorrect credentials must never trigger a paid call. Use zero minimum instances and a small maximum instance count. The first deployment created revision `chld-adapt-00001-qf6`; the validated runtime is `d1931fa`.

The application source now exists as a Next 16 / React / TypeScript app with Basic Auth-protected UI/API routes, browser-session drafts and actual PDF generation. The current local check set includes 12 unit tests and 6 browser tests. M2 found a live math numeric-change bug; runtime numeric guards and separated plan cautions from student instructions corrected it. The original failing case and a fresh live case both passed on local retest, with evidence recorded in `output/live-local-maths-retest.json`. Hosted reading and mathematics both passed at 17:16:01 UTC on runtime `d1931fa`; real A4 PDFs were one page each, visually inspected with no clipping, and SHA-verified against reviewed/downloaded bytes.
